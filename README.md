...
- **`TMDB_API_KEY` não está configurada nem localmente nem em produção** — mesmo comportamento gracioso do item acima, só que pra fonte de filmes: `GET /catalog/events?source=TMDB` retorna `503` ("Catálogo indisponível: TMDB_API_KEY não configurada") até a chave (gratuita, em [themoviedb.org](https://www.themoviedb.org/settings/api)) ser obtida e configurada. A aba "Filmes" do wizard do organizador, a troca de placeholder e os campos de local/data do passo 2 já foram verificados no navegador — só a busca em si (que depende da chave) não pôde ser testada ao vivo neste ambiente.
