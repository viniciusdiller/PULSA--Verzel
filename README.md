...
- **`TICKETMASTER_API_KEY` não está configurada em produção no momento** — a conta criada em developer.ticketmaster.com nunca teve o e-mail de confirmação entregue (verificado, não chegou), então não há chave válida pra colocar no Render ainda. Na prática: a busca no catálogo pelo organizador (`GET /catalog/events/search`) retorna `503` com a mensagem "Catálogo indisponível: TICKETMASTER_API_KEY não configurada" — é uma falha graciosa e prevista (ver `backend/src/catalog/catalog.service.ts`), não um crash. Todo o resto da plataforma funciona normalmente, incluindo os eventos já publicados pelo seed, que usam uma fixture local capturada de uma resposta real da Ticketmaster (`backend/prisma/seed.ts`) e não dependem da API estar acessível.
