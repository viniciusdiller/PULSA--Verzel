...
### Histórico operacional da portaria

O histórico da portaria é uma leitura derivada das validações persistidas em `Ticket.usedByGateUserId`, não uma segunda tabela de auditoria. `GET /gate/history/events` agrupa somente ingressos `USED` validados pelo usuário autenticado, ordena pelo uso mais recente e pagina o resultado. O filtro textual é aplicado em `Event.title` no servidor antes da agregação, evitando que a interface filtre apenas os quatro eventos já carregados.

A exportação `GET /gate/history/events/export` reutiliza o mesmo escopo de autorização, mas sem paginação: ela sempre filtra pelo atendente autenticado e retorna apenas campos operacionais mínimos em CSV. A rota usa `GATE_STAFF`, `ApiProduces('text/csv')` e um throttle próprio. O serializador escapa aspas, vírgulas e quebras de linha e neutraliza prefixos de fórmula (`=`, `+`, `-`, `@`), pois um arquivo aberto em planilha é uma fronteira de entrada de dados, não apenas uma resposta de texto. O frontend cria e revoga a URL temporária do Blob; nenhum token QR, JWT, email ou identificador interno é exportado.

## Estrutura de pastas

**Backend** (`backend/src/`): `common/{decorators,guards,filters,utils,throttling}` (`public.decorator.ts`, `roles.decorator.ts`, `current-user.decorator.ts`, `jwt-auth.guard.ts`, `roles.guard.ts`, `rate-limit.constants.ts`), `config/` (validação de env), `prisma/{prisma.module.ts,prisma.service.ts}`, módulos por feature: `auth/`, `events/`, `catalog/` (proxy Ticketmaster), `reservations/` (hold+pagamento mock, mesma transação), `tickets/` (assinatura/verificação QR, "meus ingressos", share link), `gate/` (validação, 4 status, histórico filtrável e exportação CSV). `prisma/schema.prisma`, `prisma/migrations/`, `prisma/seed.ts`.

**Frontend** (`frontend/src/`): route groups por papel — `app/(public)/{page.tsx, events/[slug]/page.tsx, events/[slug]/checkout/page.tsx, t/[shareSlug]/page.tsx, login/page.tsx}`, `app/(customer)/my-tickets/page.tsx`, `app/(organizer)/organizer/{page.tsx,new/page.tsx,[eventId]/page.tsx}`, `app/(gate)/gate/page.tsx`. `components/{ui (shadcn), seatmap, events, tickets, gate}`, `hooks/{use-gate.ts,...}`, `lib/{api-client.ts,auth.ts,download-file.ts}`. Gating de rota no `middleware.ts` é só UX — autorização real sempre no backend via guards (deixar isso explícito no README para não parecer fronteira de segurança).
...
## Painel financeiro do organizador + auditoria do Swagger (16/08)

Item que estava documentado como "corte consciente" em "O que não foi implementado e por quê" (seção acima) — o usuário pediu pra implementar de fato. `GET /events/organizer/stats` combina `reservation.groupBy({by: ['eventId'], where: {status: PAID}})` com a lista completa de eventos do organizador, pra eventos sem nenhuma venda aparecerem com R$ 0,00 em vez de sumir (groupBy sozinho só devolve quem já vendeu algo). "Vendido" é reserva `PAID`, a mesma fonte de verdade do resto do fluxo de pagamento — não o status do `Ticket`, que existe por outro motivo (validação na portaria).

**Pedido explícito de usar o MCP do 21st.dev antes de desenhar.** Busquei "organizer revenue dashboard stat cards analytics" e explorei o código de um componente ("Weekly KPI Chart" — linhas verticais por dia, com um pill de valor no item selecionado). Não colei: o padrão original é pra um número fixo de categorias (7 dias da semana); adaptei o conceito (barra que cresce a partir de uma base, valor em destaque) pra uma lista horizontal ranqueada, que aguenta N eventos com título de tamanho variável. O resumo agregado (receita total, ingressos, ticket médio) reaproveita a mesma superfície "documento" com gradiente sutil já usada no cartão de perfil, em vez de 3 KPI cards genéricos lado a lado. Cor de preenchimento das barras: violeta, não coral — coral é reservado só pra CTA em todo o app (ver seção da identidade PULSA), violeta já é a cor de "estado de dado" usada no mapa de assentos.

**Bug real de animação encontrado testando no navegador, não lendo o código.** A barra de receita usava `motion.div` com `initial={{width: 0}}` → `animate={{width: "X%"}}` — parecia certo, `npm run build`/`tsc` não acusavam nada, mas no navegador real a barra nunca saía de 0% (confirmado inspecionando o estilo computado via DOM, não só olhando a tela). Tentei trocar pra `scaleX` (mais recomendado pra esse tipo de animação em geral) — mesmo resultado, still stuck em 0. Em vez de continuar investigando a fundo uma peculiaridade específica dessa combinação de versão do `motion`/Turbopack, troquei a abordagem inteira: uma barra em CSS puro (`transition-[width]` disparada por um estado "montado" via `useEffect`/`setTimeout`), mais simples e sem depender de nenhuma biblioteca de animação — confirmada funcionando via `getComputedStyle` real no navegador antes de considerar resolvido.

**Verificação de acesso, também ao vivo e não só lendo o guard.** Bati direto na API com token de cada um dos 4 papéis: sem token → `401`, organizador → `200`, cliente → `403`, portaria → `403`. No frontend, tentei acessar `/organizer/finance` logado como cada papel: cliente cai em `/my-tickets`, portaria em `/gate`, deslogado em `/login` — a rota herda `RouteGuard allow={["ORGANIZER"]}` do layout `(organizer)/`, então não precisei adicionar proteção nenhuma nova, só confirmar que o layout existente realmente cobre a rota nova.

**Auditoria de cobertura do Swagger, motivada por outra pergunta do usuário.** Em vez de só contar `@ApiOperation` no código-fonte (o que pode mentir se o decorator existir mas o Swagger não processar por algum motivo), subi o backend e baixei o JSON real de `/api-docs-json`, contando `summary`/`tags` por operação. Achado real: `GET /health` era a única das 32 rotas sem `@ApiOperation` — o resto (inclusive a rota nova do painel financeiro) já estava 100% documentado. Corrigido e reconferido no JSON gerado depois do fix.

## Keep-alive falhando em produção: retry pro cold start (17/08)

O workflow de keep-alive (ver seção "Auditoria final e limpeza de Railway") começou a rodar de verdade assim que a branch com ele foi mergeada em `main` — GitHub Actions só dispara agendamentos a partir do branch padrão, então até o merge ele ficava inerte. Primeira execução real: falhou. O `curl -fsS --max-time 55` de tentativa única não tinha margem nenhuma — testei o mesmo endpoint manualmente logo depois e respondeu `200` normalmente, então não era o backend que estava fora do ar, era o cold start ocasionalmente passando dos 55s de limite (a soma de acordar o serviço no Render + latência de rede do runner do GitHub até lá). Trocado por até 3 tentativas de 60s cada, com 10s de pausa entre elas — a primeira tentativa já serve pra disparar o cold start mesmo se estourar o tempo; as seguintes pegam o serviço já acordando ou acordado.

## Cancelamento de ingresso já pago, com estorno em saldo (17/08, branch `feat/cancel-paid-ticket-refund`)

Item que já tinha aparecido numa conversa como corte consciente ("só existe cancelamento pré-pagamento") — o usuário pediu pra implementar. Decisão de arquitetura: método novo `ReservationsService.cancelPaid()` e rota nova `POST /reservations/:id/cancel-paid`, em vez de estender o `cancel()` existente (que hoje só cuida de desistir de um *hold*, antes de pagar) pra também aceitar reservas `PAID`. Os dois fluxos têm regras de elegibilidade e efeitos colaterais diferentes o bastante (um libera um hold; o outro invalida um ticket já emitido e credita saldo) que unificar os dois métodos exigiria reescrever/quebrar os testes já existentes de `cancel()` sem ganho real — separar manteve o fluxo de hold intocado e testado, e deixou a lógica nova isolada.

**Três regras de elegibilidade**, checadas antes de tocar no banco: a reserva precisa estar `PAID`, o ticket emitido precisa ainda estar `VALID` (não dá pra estornar um ingresso já usado na portaria), e o evento não pode já ter acontecido. Dentro da transação, os dois updates que importam (`reservation` e `ticket`) usam a mesma estratégia de "update condicional + checar linhas afetadas" já usada em `cancel()`/`gate` — cobre o caso raro de a portaria validar o ingresso entre a leitura e o cancelamento (a transação inteira reverte sozinha se o `ticket.updateMany` não afetar nenhuma linha). Reembolso: sempre o valor total em saldo, mesma convenção já usada quando um organizador cancela um evento inteiro (`cancelWithRefund`) — não importa se o pagamento original foi só cartão, só saldo, ou uma mistura.

**Frontend**: botão "Cancelar ingresso" dentro de `TicketCard`, atrás de um prop `allowCancel` que só "Meus ingressos" liga — esse mesmo componente é reaproveitado na página pública de compartilhamento (`/t/[shareSlug]`, sem login), então o botão precisa ficar fora por padrão em vez de aparecer condicionalmente por dentro. Pesquisei padrões de confirmação/cancelamento no MCP do 21st.dev antes de desenhar (inclusive um padrão de "digite pra confirmar" pra ações destrutivas), mas decidi reaproveitar o `Dialog` de confirmação que o app já usa pra "Excluir evento" em vez de introduzir um padrão novo só pra essa tela — mesmo critério já usado no painel financeiro (adaptar referência externa quando ajuda, mas priorizar consistência com o que o próprio app já estabeleceu).

**Achado real de lint, não de teste**: `new Date(ticket.event.startsAt).getTime() <= Date.now()` direto no corpo do componente disparava a regra `react-hooks/purity` do eslint-config-next 16 (`Date.now()` é impuro, não pode rodar toda vez que o componente renderiza). Resolvido com o mesmo truque já usado em `use-countdown.ts`: calcular uma vez dentro do inicializador preguiçoso de um `useState`, que roda só na montagem — e como cada `TicketCard` é montado com uma key estável por ticket, isso já é suficiente.

**Verificação ao vivo, não só testes automatizados**: logada como `cliente1@elitedev.dev`, cancelei de verdade um ingresso válido de um evento futuro pela UI — conferi o toast mostrando o valor exato reembolsado (R$ 50,00), o saldo subindo na hora de 44550 para 49550 centavos via `GET /auth/me`, o ticket virando `VOID` e o assento voltando pra `AVAILABLE` no seatmap, tudo batendo com o banco real via chamadas diretas à API, não só a tela.

## Auditoria completa de segurança (17/08, branch `security/audit-hardening`)

Detalhamento completo (tabela de achados + o que já estava certo) em `docs/SECURITY_AUDIT.md` — aqui só o racional das decisões mais arquiteturais.

**O achado mais interessante não veio de ler código, veio de medir.** A hipótese de que o login vazava enumeração de usuários por tempo de resposta (bcrypt só rodava quando o email existia) é óbvia lendo o código, mas só virou um achado real depois de cronometrar `curl` contra o endpoint de verdade: ~3ms sem usuário vs. ~250ms com usuário+senha errada. Depois do fix (bcrypt.compare sempre roda, contra um hash fixo quando não existe usuário), a diferença ainda não tinha sumido completamente — foi aí que apareceu um segundo problema, esse sim só visível comparando os números: o hash fixo usava 10 rounds de bcrypt, mas o seed usa 12, e comparar contra um custo diferente do custo real reabre uma versão mais sutil do mesmo canal lateral. Unificado pra 12 rounds em todo lugar (o hash fixo e `updateProfile`, que também estava em 10).

**A dependência vulnerável (`js-yaml`, via `@nestjs/swagger`) foi um caso de "a correção óbvia piora o problema".** `npm audit` sugeria `npm audit fix --force`, que trocaria pra uma versão mais antiga do `@nestjs/swagger` — só que essa versão mais antiga carrega `js-yaml@4.3.0`, que tem OUTRO CVE (diferente do `js-yaml@5.2.1` usado na versão atual). As duas opções "óbvias" (ficar ou trocar de versão do swagger) são vulneráveis a algum CVE. A saída foi um `overrides` no `package.json` forçando `js-yaml@4.3.1` (a única versão sem nenhum dos dois problemas) só dentro da árvore do `@nestjs/swagger`, mantendo o `@nestjs/swagger` na faixa normal (`^11.4.6`) — resolve o problema real sem prender o resto do dependency tree numa versão antiga pra sempre. `npm audit` limpo depois, Swagger (`/api-docs`, `/api-docs-json`) testado ao vivo pra confirmar que a doc da API continua sendo gerada normalmente.

**Decisão consciente de não adicionar CSP no frontend.** O backend já desativa CSP no Helmet por decisão documentada (API pura, sem HTML de terceiros pra proteger); o frontend nunca teve nenhum header de segurança, então adicionei os básicos (`X-Frame-Options`, `X-Content-Type-Options`, `Referrer-Policy`, HSTS) mas deixei CSP de fora de propósito — a portaria depende de `getUserMedia` (câmera, via `html5-qrcode`) pra ler QR, e calibrar um CSP que não quebre isso, mais a hidratação do Next, mais o `motion`, exigiria testar cada fluxo da aplicação de novo dentro do escopo desta auditoria. Documentado como corte consciente, não esquecimento — mesmo padrão já usado no resto do projeto pra decisões de escopo.

## SEO básico: sitemap e robots.txt (17/08, branch `feat/seo-basics`)

Iniciativa própria, não pedida pelo desafio — `app/sitemap.ts` e `app/robots.ts` usando `MetadataRoute` nativo do App Router, sem nenhuma dependência nova. Sitemap pagina `GET /events` (rota pública, sem auth) até acabar e lista a home + a URL de cada evento publicado; robots.txt libera o catálogo público e bloqueia as áreas que exigem login (sem valor de indexação, e `/t/` — link de compartilhamento de ingresso — nem deveria aparecer numa busca).

Achado real testando localmente sem o backend rodando: o `fetch` dentro do `sitemap.ts` derrubava o build inteiro (`TypeError: fetch failed`), porque a geração roda no servidor e sitemap/robots são pré-renderados por padrão. Dois ajustes: fallback silencioso (sitemap cai pra só a home se a API não responder, em vez de quebrar a rota) e `export const revalidate = 3600`, porque sem isso o sitemap só seria gerado uma vez no build — um evento publicado depois do último deploy nunca apareceria até o próximo deploy. Confirmado ao vivo com `curl` contra `/robots.txt` e `/sitemap.xml` (inclusive o caminho de fallback, backend desligado de propósito) antes de considerar pronto.

**Segunda rodada, a pedido do usuário ("não tem onde melhorar esse SEO?").** Recusei meta tag de "keywords" — o Google ignora esse campo desde 2009, adicionar seria trabalho sem nenhum efeito real, e ponytail existe justamente pra não construir o que não funciona. O que valia a pena: toda página do site (inclusive a de cada evento) usava o mesmo `title`/`description` genéricos do layout raiz, e a página de evento — a única com valor de SEO individual real, já que o resto do site é genérico (home) ou exige login — não tinha nenhum dado estruturado pro Google entender.

`app/(public)/events/[eventId]/page.tsx` virou Server Component com `generateMetadata` (busca o evento via `fetch` direto, mesmo padrão resiliente do `sitemap.ts`) — a lógica antiga inteira foi só movida pra `event-detail-client.tsx`, sem reescrever nada. Título/descrição por evento, OpenGraph, e JSON-LD `schema.org/Event` (nome, data, local, preço) — é o formato que o Google reconhece pra mostrar esses dados direto no resultado de busca, sem a pessoa precisar clicar. `metadataBase` adicionado no layout raiz de passagem (resolve URL de imagem OG corretamente, evita o warning do Next). Mesmo cuidado de resiliência do sitemap: testado ao vivo com o backend desligado de propósito, a rota responde `200` normal e só omite o JSON-LD.

## Aviso de cookies + política de privacidade (17/08, branch `feat/cookie-consent`, a partir de `feat/seo-basics`)

Pedido pra seguir "o padrão da indústria" com craft de front-end/design de verdade, não um banner cinza genérico de "usamos cookies" — a cara de AI slop que o resto do projeto evita. Banner flutuante no canto (acima do `MobileBottomNav` no mobile via `bottom-20`, `bottom-4` a partir de `sm:`) com a linha perfurada de "canhoto de ingresso" já usada em outras partes do app, dois botões ("Somente essenciais" / "Aceitar") e link pra uma página de política de privacidade nova (`/privacidade`), que também entrou no `sitemap.ts` com prioridade baixa.

**Dois bugs reais, achados testando no navegador, não lendo o código:**

1. **Lint, não runtime.** Primeira versão lia a escolha salva com `useState` + `useEffect` (`useEffect(() => setVisible(!getStored()), [])`) — padrão óbvio, mas dispara a regra `react-hooks/set-state-in-effect` do `eslint-config-next 16`. Reescrito com `useSyncExternalStore` (`lib/cookie-consent.ts` expõe `subscribe`/`getSnapshot`/`getServerSnapshot`) — o padrão que o próprio React recomenda pra ler uma store externa ao componente (aqui, `localStorage`) sem lint e sem risco de mismatch de hidratação, já que o servidor não tem acesso a `localStorage` e trata como "ainda não decidiu" (`getServerSnapshot` sempre retorna `null`).
2. **Mesma classe de bug já documentada em `PendingCancellationNotice`, encontrada de novo.** Com `AnimatePresence` envolvendo o `motion.div`, clicar em "Aceitar" persistia a escolha corretamente (confirmado lendo `localStorage` direto) e disparava a animação de saída de verdade (o `opacity` computado chegava a `"0"`, não é só a UI "parecer" ter sumido) — mas o nó nunca era removido do DOM, ficando invisível e potencialmente bloqueando cliques na área. Mesmo fix do outro componente: tirar o `AnimatePresence`, renderização condicional direta (`if (!visible) return null`) antes do JSX, perdendo só a animação de saída (mantém a de entrada). Essa combinação específica de versões (Next 16 + Turbopack + `motion`) já apareceu nesse projeto o bastante pra virar um padrão conhecido — qualquer elemento novo condicionalmente renderizado que precise somar/sumir deveria evitar `AnimatePresence` de saída por padrão, não só quando o bug aparecer de novo.

**Política de privacidade** (`app/(public)/privacidade/page.tsx`) escrita com o conteúdo real do projeto, não um texto genérico de gerador — os três usos de `localStorage` (sessão, tema, esta escolha), a ausência de qualquer rastreamento de terceiros, o fato de o pagamento ser simulado, que as chaves de API externas (Ticketmaster/TMDb) ficam só no servidor, e uma nota explícita de que é um projeto de demonstração pra um desafio técnico, não uma empresa real.

Cinco testes novos em `lib/cookie-consent.test.ts` (mesmo padrão de `auth.test.ts`: `localStorage` limpo a cada teste) cobrindo leitura/escrita da escolha, `getServerSnapshot` sempre `null`, valor inválido salvo por fora da lib sendo tratado como "sem escolha", e notificação de listeners inscritos via `subscribeConsent` — frontend foi de 42 pra 53 testes.

## Keep-alive definitivo: troca do cron do GitHub Actions por UptimeRobot (18/08, branch `chore/uptimerobot-keep-alive`)

O `keep-alive.yml` (ver seção "Keep-alive falhando em produção" acima) continuava falhando em produção mesmo depois do retry — o usuário confirmou que o cold start ainda acontecia com frequência na avaliação. Investigando a causa raiz: não era o `curl`/retry que estava errado, era a premissa de usar `cron` do GitHub Actions pra esse tipo de tarefa. O próprio GitHub documenta que workflows agendados **não têm garantia de horário de disparo** — em períodos de carga alta da plataforma o atraso pode passar bem de 15-20min, o que é maior que a janela de hibernação do Render (~15min). Ou seja: o workflow em si estava correto, mas a ferramenta escolhida (cron do Actions) é estruturalmente inadequada pra manter um serviço acordado com precisão de minutos — não tem retry que resolva isso, porque o problema é o disparo em si atrasando, não a requisição falhando.

**Antes de trocar, uma pergunta útil do usuário: dava pra resolver de dentro do próprio backend, sem depender de nada externo?** Não, e vale registrar o porquê: hibernar no plano gratuito do Render não é o processo ficar ocioso na rede — é o **container inteiro sendo desligado**. Não sobra nenhum `setInterval`/worker rodando pra se auto-pingar, porque não existe processo Node executando nesse momento. A única coisa que acorda o serviço é uma requisição HTTP chegando de fora; um self-ping interno é logicamente impossível aqui (o processo precisaria estar vivo pra se manter vivo).

**Solução**: removido `.github/workflows/keep-alive.yml`, substituído por um monitor HTTP no [UptimeRobot](https://uptimerobot.com) (free tier) apontando pra `/api/health` a cada 5 minutos — um serviço feito especificamente pra esse tipo de ping, com scheduler próprio e confiável (não compartilha o problema de fila do Actions). Configuração feita direto no dashboard do UptimeRobot pelo usuário (não há nada pra versionar no repositório além da remoção do workflow antigo) — fora do escopo do que a IA tem acesso, já que exige criar conta numa conta de terceiros.
